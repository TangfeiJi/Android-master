...
public class PickPayDayView extends LinearLayout implements NumberPicker.OnValueChangeListener {
    ...
    List<PayrollType> list;
    /**
     * 回调接口对象
     */
    private onSelectedChangeListener mSelectedChangeListener;

    public PickPayDayView(Context context) {
        super(context);
        this.mContext = context;
        generateView();
    }

    public PickPayDayView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        generateView();
    }

    public PickPayDayView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        }
    ...
    private void initRightViewAndPicker() {
        /**
         * 右侧控件
         */
    ...
    private void updateLeftView(Object value) {
        ...
        for(int i=0; i<list.size(); i++) {
            String key = list.get(i).getPayKey().getName();
            if(key.equals(value)) {
                Object[] mRight = new Object[list.get(i).getPayValue().size()];
                List<PayrollDetails> l = list.get(i).getPayValue();
                for (int j = 0; j < l.size(); j++) {
                    mRight[j] = l.get(j).getName();
                }
                this.mRightValues = mRight;
            }
        }
        initRightViewAndPicker();
    }
     ...
     * 设置数据--单列数据
     */
    public void setValueData(List<PayrollType> list) {
        this.list =list;
        Object[] mLeft = new Object[list.size()];
        for (int i = 0; i < list.size(); i++) {
            mLeft[i] = list.get(i).getPayKey().getName();
        }
        this.mViewCount = 2;
        this.mLeftValues = mLeft;
    ...
    public interface onSelectedChangeListener {
        void onSelected(PickPayDayView view, Object leftValue, Object rightValue);
    }
}
